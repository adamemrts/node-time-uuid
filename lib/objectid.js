...
var ObjectId = module.exports = function(bytes) {
    ...
    self.init = function(bytes) {
        if (Buffer.isBuffer(bytes)) {
            ...
            var dateWrapper = new Date(bytes);
            if (!isNaN(dateWrapper.getDate())) {
                var seconds = parseInt(bytes.toString().slice(0, 10));
                var microSeconds = parseInt(bytes.toString().slice(10, 16));
                // properly padEnd a number
                if (microSeconds < 1e5) microSeconds = parseInt((microSeconds.toString() + '000000').slice(0, 6));
            ...
            throw new Error("Number-based ObjectId must be timestamp in Unix or JavaScript format.");
        }
